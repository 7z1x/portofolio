...
import { SiKotlin, SiPython, SiJavascript, SiDocker, SiFirebase, SiGit, SiReact, SiPytorch, SiTensorflow, SiJetpackcompose, SiFigma, SiPostgresql, SiFastapi } from 'react-icons/si';
...
// Parse headline into display lines — split on newline or comma
function parseHeadlineLines(headline) {
  if (!headline) return null;
  // Try splitting by newline first, fallback to comma
  const byNewline = headline.split('\n').map((s) => s.trim()).filter(Boolean);
  if (byNewline.length > 1) return byNewline;
  const byComma = headline.split(',').map((s) => s.trim()).filter(Boolean);
  return byComma.length > 1 ? byComma : [headline.trim()];
}

export default function About() {
  const siteSettings = useSiteSettings();

  const heroName = siteSettings?.name ?? 'Zulfahmi';
  const headlineLines = siteSettings?.headline
    ? parseHeadlineLines(siteSettings.headline)
    : ['A Creative Developer', '& A Curious Thinker.'];

  const socialLinks = {
    instagram: siteSettings?.instagramUrl ?? 'https://instagram.com/llzf4_',
    github: siteSettings?.githubUrl ?? 'https://github.com/7z1x',
    linkedin: siteSettings?.linkedinUrl ?? 'https://www.linkedin.com/in/zulfahmi-m-ardianto/',
  };

  useEffect(() => {
    document.title = 'Zulfahmi M Ardianto | AI Engineer & Creative Developer';
  }, []);
          ...
          <div className="about-hero-section">
            ...
            <div className="hero-text-container">
                ...
                <div className="hero-glass-content">
                  <h1 className="hero-title">
                    <span className="hero-title-highlight">
                      <DecryptedText text={`${heroName} is a`} animateOn="view" speed={60} sequential={true} />
                    </span>
                    {headlineLines.map((line, i) => (
                      ...
                      >
                        <DecryptedText text={line} animateOn="view" speed={60} sequential={true} />
                      </span>
                    ))}
                  </h1>
                </div>
              ...
              <div className="hero-actions-row">
                <a
                  className="cv-button"
                  href="/Zulfahmi_M_Ardianto_Resume_ID.pdf"
                  target="_blank"
                ...
                <div className="hero-socials">
                  <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="social-icon social-ig" aria-label="Instagram">
                  ...
                  <a href={socialLinks.github} target="_blank" rel="noopener noreferrer" className="social-icon social-gh" aria-label="GitHub">
                  ...
                  <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="social-icon social-li" aria-label="LinkedIn">
